...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    shift
    PEER_ORGS_INFO=("$@")
    deploy_chaincode "$ORD_ORG" "$PEER_ORG" "$CHAINCODE_PATH" "$CHAINCODE_COMMITED" "${PEER_ORGS_INFO[@]}"
    ...
    approve_chaincode $@ $channel_name
    ...
    invoke_chaincode $CHAINCODE_NAME $@ 2>> ${LOG_FILE}

  elif [ "${COMMAND}" == "query" ]; then
    query_chaincode $CHAINCODE_NAME $@ >> ${LOG_FILE}
...
function deploy_chaincode() {
  ...
  if [ "${CHAINCODE_BUILDER}" == "ccaas" ]; then
    ...
    launch_chaincode      ${cc_name} ${CHAINCODE_ID} ${CHAINCODE_IMAGE} ${ORD_ORG} ${PEER_ORG} "${PEER_ORGS_INFO[@]}"
  fi

  activate_chaincode ${cc_name} ${cc_package} ${PEER_ORG} ${ORD_ORG} $CHAINCODE_COMMITED "${PEER_ORGS_INFO[@]}" 
}
...
function activate_chaincode() {
  ...
  for PEER_ORG_INFO in "${PEER_ORGS_INFO[@]}"; do
    ...
    for PEER in "${PEERS[@]}"; do
      install_chaincode ${cc_package} ${PEER_ORG} $PEER
      approve_chaincode ${cc_name} ${CHAINCODE_ID} ${PEER_ORG} ${ORD_ORG} $PEER
...
function query_chaincode() {
  ...
  export_peer_context $peer_org peer1
...
function invoke_chaincode() {
  ...
  peer chaincode invoke \
    ...
    -C              $CHANNEL_NAME \
    -c              $2 \
    --orderer       $ord_org-orderer1.${DOMAIN}:${NGINX_HTTPS_PORT} \
    --connTimeout   ${ORDERER_TIMEOUT} \
    --tls --cafile  ${TEMP_DIR}/channel-msp/ordererOrganizations/$ord_org/orderers/$ord_org-orderer1/tls/signcerts/tls-cert.pem
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $NETWORK_NAME apply -f -

  kubectl -n $NETWORK_NAME rollout status deploy/${org}${peer}-ccaas-${cc_name}
...
function launch_chaincode() {
  ...
  for PEER_ORG_INFO in "${PEER_ORGS_INFO[@]}"; do
    ...
    IFS=',' read -ra PEERS <<< "${PEER_ORG_ARRAY[1]}"
    for PEER in "${PEERS[@]}"; do
        launch_chaincode_service ${PEER_ORG} $PEER ${cc_name} ${cc_id} ${cc_image}
    done
  done  
}
...
function install_chaincode() {
  ...
  install_chaincode_for ${org} ${peer} ${cc_package}
}
...
function approve_chaincode() {
  ...
  export_peer_context $peer_org $peer
